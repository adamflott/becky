...
impl SystemConfiguration {
    ...
    pub async fn new(run_path: PathBuf, emulator_paths: Vec<PathBuf>, vm_root_path: PathBuf, vm_data_root_path: PathBuf, os_cache_root_path: PathBuf) -> Self {
        ...
        let _ = tokio::fs::create_dir_all(&run_path).await;
        debug!("created path {:?}", run_path);
        let _ = tokio::fs::create_dir_all(&vm_root_path).await;
        debug!("created vm run path {:?}", vm_root_path);
        let _ = tokio::fs::create_dir_all(&vm_data_root_path).await;
        debug!("created vm data path {:?}", vm_data_root_path);
        let _ = tokio::fs::create_dir_all(&os_cache_root_path).await;
